...
{
  std::cout << "Is started: " << is_started_ << std::endl;
}
...
void Vision::config_rgb_camera(std::string camera_name, std::string frame_id)
{
  std::cout << "Loading rgb camera config" << std::endl;
  camera_type_ = CameraTypes::Color;
  image_encoding_ = "CV_8UC3";  // color image with red - green - blue color order
  pixel_size_ = 3;
...
bool Vision::initialize()
{
  ...
  {
    std::cerr << "Error:: gst_pipeline_ is NULL. Exiting." << std::endl;
    return false;
  }
  ...
  if (GST_IS_PIPELINE(gst_pipeline_))
  {
    ...
    {
      std::cerr << camera_name_ << " : gst_bin_add() failed" << std::endl;
    ...
    {
      std::cerr << camera_name_ << ": gstreamer: cannot link outelement" << gst_element_get_name(outelement)
                << " -> sink" << std::endl;
      gst_object_unref(outelement);
      gst_object_unref(gst_pipeline_);
      gst_pipeline_ = NULL;
  ...
  else
  {
    ...
    {
      std::cerr << camera_name_ << ": gstreamer: cannot link launchpipe -> sink" << std::endl;
      gst_object_unref(gst_pipeline_);
      gst_pipeline_ = NULL;
      return false;
    }
  ...
  {
    std::cerr << camera_name_ << "Failed to PAUSE stream, check your gstreamer configuration." << std::endl;
    return false;
  }
  else
  {
    std::cerr << camera_name_ << " Stream is PAUSED " << std::endl;
  }
...
bool Vision::start()
{
  ...
  switch (ret)
  {
    ...
    case GST_STATE_CHANGE_NO_PREROLL:
      std::cerr << camera_name_ << "Failed to start stream" << std::endl;
      return false;
    ...
    case GST_STATE_CHANGE_ASYNC: {
      ...
      switch (ret)
      {
        ...
        case GST_STATE_CHANGE_ASYNC:
          std::cerr << camera_name_ << "  Failed to start stream (timeout)" << std::endl;
          return false;
      }
    }

    case GST_STATE_CHANGE_SUCCESS:
      std::cerr << camera_name_ << "  Stream started" << std::endl;
      break;
...
//   {
//     ROS_INFO("[%s]: Custom camera information file not set, using default one based on sensor
//     resolution",
//              camera_name_.c_str());
...
//     else
//     {
//       ROS_WARN("[%s]: Camera info at '%s' not found. Using an uncalibrated config.",
//       camera_name_.c_str(),
//                camera_info_.c_str());
//     }
//   }
...
//   {
//     ROS_WARN_ONCE("[%s]: Calibration file sensor resolution (%dx%d pixels) doesn't match stream
//     resolution
//     (%dx%d
//     "
//                   "pixels)",
...
//   {
//     ROS_WARN_ONCE("[%s]: Image buffer underflow: expected frame to be %u bytes but got only %lu bytes.
//     Make sure
//     "
...
//   if (buf_size > expected_frame_size)
//   {
//     ROS_WARN_ONCE("[%s]: Image buffer overflow: expected frame to be %u bytes but got %lu bytes. Make
//     sure "
//                   "frames are correctly encoded.",
//                   camera_name_.c_str(), expected_frame_size, buf_size);
...
bool Vision::changePipelineState(GstState state)
{
  ...
  switch (ret)
  {
    ...
    case GST_STATE_CHANGE_FAILURE:
      std::cerr << camera_name_ << " Failed to change pipeline state to " << gst_element_state_get_name(state)
                << std::endl;
      return false;
    ...
    default:
      std::cerr << camera_name_ << "  Unknown state change return value when trying to change pipeline state to "
